...
const processCascadeSelect = (item: any) => {
  bibliographySelection.value = store.bibliography.filter(x => x[item.value?.parent ? 'region' : 'district'] === item.value.name);
};
...
<template>
  ...
  <div class="flex justify-content-between flex-wrap mb-4" style="max-width:250px;">
    <div class="flex align-items-center justify-content-center mr-2 font-bold">Ад укладальніка</div>
  ...
  <template v-for="(item, key) in bibliographySelection" :key="key">
    <div
      v-if="!key || (key && bibliographySelection[key - 1]['title'] !== item.title)"
      class="shadow-8 item"
    >
